...
const MODULE_VERSION = "2026-08-22-b2b-qty-remove-merge-dryrun-v1.1.0";
const ROUTE = "/amazon/price/b2b/quantity/remove/merge/dry-run";
...
function getConfig() {
  ...
  if (!marketplaceId) throw new Error("Missing env: SPAPI_MARKETPLACE_ID");
...
function analyzeListing(listing) {
  ...
  return {
    ...
    b2bSelector: {
      audience: String(b2b?.audience || "").toUpperCase(),
      currency: String(b2b?.currency || "").toUpperCase(),
      marketplaceId: String(b2b?.marketplace_id || ""),
    },
    configuredMarketplaceId: marketplaceId,
  };
}
...
function normalizeItems(body) {
  ...
  return items.map((item, index) => {
    ...
    if (!Number.isInteger(expectedB2bPrice) || expectedB2bPrice <= 0) throw new Error(`items[${index}].expectedB2bPrice must be a positive integer`);
    if (ssotQuantityDiscountEnabled !== false) throw new Error(`items[${index}].ssotQuantityDiscountEnabled must be false`);
    if (!Number.isInteger(ssotQuantityMinLot) || ssotQuantityMinLot <= 0) throw new Error(`items[${index}].ssotQuantityMinLot must be a positive integer`);
    if (!expectedQuantityDiscountType) throw new Error(`items[${index}].expectedQuantityDiscountType is required`);
    if (!expectedQuantityTiers.length) throw new Error(`items[${index}].expectedQuantityTiers must be non-empty`);
    ...
    return {
      ...
      ssotQuantityMinLot,
      expectedQuantityPlan: { discountType: expectedQuantityDiscountType, tiers: expectedQuantityTiers },
    };
  });
}
...
function guardState(item, state) {
  ...
  if (state.b2bPrice !== item.expectedB2bPrice) errors.push(`B2B price mismatch: expected=${item.expectedB2bPrice} actual=${state.b2bPrice}`);
  if (!(state.availableQuantity < item.ssotQuantityMinLot)) errors.push(`inventory no longer below SSOT quantity minimum lot: available=${state.availableQuantity} minLot=${item.ssotQuantityMinLot}`);
  if (!plansEqual(state.quantityPlan, item.expectedQuantityPlan)) errors.push(`quantity plan mismatch: expected=${JSON.stringify(item.expectedQuantityPlan)} actual=${JSON.stringify(state.quantityPlan)}`);
  if (state.b2bSelector.audience !== "B2B") errors.push(`B2B audience selector invalid: ${state.b2bSelector.audience}`);
  if (!state.b2bSelector.currency) errors.push("B2B currency selector missing");
  if (!state.b2bSelector.marketplaceId) errors.push("B2B marketplace_id selector missing");
  if (state.b2bSelector.marketplaceId !== state.configuredMarketplaceId) {
    errors.push(`B2B marketplace_id mismatch: configured=${state.configuredMarketplaceId} actual=${state.b2bSelector.marketplaceId}`);
  }
  if (errors.length) {
    const err = new Error(`B2B quantity-remove merge dry-run preflight failed: ${errors.join(" / ")}`);
...
function buildMergeNullPatch(state) {
  const { audience, currency, marketplaceId } = state.b2bSelector;
  if (audience !== "B2B" || !currency || !marketplaceId) throw new Error("B2B selector fields incomplete");
  return {
...
async function handler(req, res) {
  ...
  try {
    ...
    if (!secret) return res.status(500).json({ ok: false, moduleVersion: MODULE_VERSION, route: ROUTE, requestedAt, externalChanges: 0, error: "AMAZON_STOCK_API_SECRET is not set" });
    if (String(req.headers["x-api-secret"] || "") !== secret) return res.status(401).json({ ok: false, moduleVersion: MODULE_VERSION, route: ROUTE, requestedAt, externalChanges: 0, error: "Unauthorized" });
    ...
    for (let i = 0; i < items.length; i += 1) {
      ...
      try {
        ...
        const patchBody = buildMergeNullPatch(state);
    ...
    console.error("B2B quantity-remove merge dry-run error", err?.message || String(err));
    return res.status(400).json({ ok: false, moduleVersion: MODULE_VERSION, route: ROUTE, requestedAt, externalChanges: 0, error: err?.message || String(err) });
  }
}

express.application.listen = function b2bQuantityRemoveMergeDryRunListen(...args) {
