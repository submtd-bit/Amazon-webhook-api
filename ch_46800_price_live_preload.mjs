...
const MODULE_VERSION = "2026-08-30-ch-46800-price-v1.0.0";
const DRY_ROUTE = "/amazon/price/ch-46800/dry-run";
const LIVE_ROUTE = "/amazon/price/ch-46800/live";
const LIVE_CONFIRM = "CH_46800_LIVE_APPROVED_20260830";
...
const TARGET = Object.freeze({
  ...
  targetNormal: 46800,
  amazonMinSsot: 37200,
  minimumSellerAllowed: 44000,
  amazonPointsBefore: 740,
  b2bAttributePrice: 70300,
  ...
  try {
    ...
    if (initialState.normalPrice === TARGET.targetNormal && initialState.actual.b2cPrice === TARGET.targetNormal) {
      ...
      return res.status(blocks.length ? 409 : 200).json({
        ...
        moduleVersion: MODULE_VERSION,
        route: DRY_ROUTE,
        decision: blocks.length ? "TARGET_ALREADY_PRESENT_BUT_PROTECTED_STATE_INVALID" : "ALREADY_APPLIED_CH_46800",
    ...
    return res.status(200).json({
      ...
      moduleVersion: MODULE_VERSION,
      route: DRY_ROUTE,
      decision: "DRY_RUN_READY_FOR_CH_46800_NO_MUTATION",
      approvedScope: {
        sku: TARGET.sku,
        asin: TARGET.asin,
        currentNormal: TARGET.currentNormal,
    ...
    console.error("CH 46800 dry-run error", err?.message || String(err));
...
async function liveHandler(req, res) {
  ...
  try {
    ...
    console.error("CH 46800 live error", err?.message || String(err));
    return res.status(500).json({
      ok: false,
      moduleVersion: MODULE_VERSION,
      route: LIVE_ROUTE,
...
express.application.listen = function ch46800PriceListen(...args) {
