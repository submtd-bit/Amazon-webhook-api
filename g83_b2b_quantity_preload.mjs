...
const MODULE_VERSION = "2026-08-18-g83-b2b-quantity-v1.0.0";
...
const TARGET_LOWER_BOUND = 10;
const TARGET_QUANTITY_PRICE = 38700;
const TARGET_DISCOUNT_TYPE = "QUANTITY_DISCOUNT";
const EXPECTED_SALE_END_UTC = "2026-08-23T15:00:00.000Z";
const LIVE_CONFIRM = "G83-B2B-Q10-38700";
...
function quantityLevels(offer) {
  const plan = offer?.quantity_discount_plan?.[0]?.schedule?.[0] || {};
  const type = String(plan?.discount_type || "").toUpperCase();
  const levels = Array.isArray(plan?.levels) ? plan.levels : [];
  return levels.map(level => ({ lowerBound: numberOrNull(level?.lower_bound), value: numberOrNull(level?.value), discountType: type })).filter(x => x.lowerBound !== null && x.value !== null);
}
function targetQuantityLevel(levels) { return levels.find(x => x.lowerBound === TARGET_LOWER_BOUND && x.discountType === TARGET_DISCOUNT_TYPE) || null; }
...
function analyzeListing(listing) {
  ...
  const qLevels = quantityLevels(b2b); const q10 = targetQuantityLevel(qLevels);
  return {
    ...
    offers, consumer, b2b,
    normalPrice: numberOrNull(normalSchedule?.value_with_tax), salePrice: numberOrNull(saleSchedule?.value_with_tax), saleEnd: isoMs(saleSchedule?.end_at),
    minPrice: numberOrNull(minSchedule?.value_with_tax), b2bPrice: numberOrNull(b2bSchedule?.value_with_tax),
    quantityLevels: qLevels, q10Price: q10 ? q10.value : null, q10LowerBound: q10 ? q10.lowerBound : null
  };
}
...
function assertPreflight(state) {
  ...
  if (state.quantityLevels.some(x => x.lowerBound !== TARGET_LOWER_BOUND)) errors.push(`unexpected existing B2B quantity tier(s): ${JSON.stringify(state.quantityLevels)}`);
  if (state.q10Price !== null && state.q10Price !== TARGET_QUANTITY_PRICE) errors.push(`existing Q10 price mismatch: ${state.q10Price}`);
  if (errors.length) { const err = new Error(`G83 B2B quantity preflight failed: ${errors.join(" / ")}`); err.code = "PREFLIGHT_FAILED"; err.details = errors; throw err; }
}
...
function buildPatch(state) {
  ...
  offers[b2bIndex].quantity_discount_plan = [{ schedule: [{ discount_type: TARGET_DISCOUNT_TYPE, levels: [{ lower_bound: TARGET_LOWER_BOUND, value: TARGET_QUANTITY_PRICE }] }] }];
...
async function submitPatch(accessToken, patchBody, validationPreview) {
  const { sellerId, marketplaceId, endpoint } = getConfig();
  const query = new URLSearchParams({ marketplaceIds: marketplaceId, issueLocale: "ja_JP" }); if (validationPreview) query.set("mode", "VALIDATION_PREVIEW");
  const url = `${endpoint}/listings/2021-08-01/items/${encodeURIComponent(sellerId)}/${encodeURIComponent(G83_SKU)}?${query}`;
  const result = await amazonRequest({ method: "PATCH", url, accessToken, body: patchBody });
  if (hasErrorIssues(result)) throw new Error(`Amazon validation returned ERROR issues: ${JSON.stringify(result.issues)}`);
  return result;
}
function makeFingerprint(state) {
  const secret = getSecret();
  const payload = { v: 1, sku: G83_SKU, asin: G83_ASIN, normalPrice: state.normalPrice, salePrice: state.salePrice, minPrice: state.minPrice, b2bPrice: state.b2bPrice, saleEnd: state.saleEnd, currentQ10: state.q10Price, targetLowerBound: TARGET_LOWER_BOUND, targetQuantityPrice: TARGET_QUANTITY_PRICE, issuedAt: Date.now() };
...
function verifyFingerprint(token) {
  ...
  if (payload.sku !== G83_SKU || payload.asin !== G83_ASIN || payload.normalPrice !== EXPECTED_NORMAL_PRICE || payload.salePrice !== EXPECTED_SALE_PRICE || payload.minPrice !== EXPECTED_MIN_PRICE || payload.b2bPrice !== EXPECTED_B2B_PRICE || payload.saleEnd !== EXPECTED_SALE_END_UTC || payload.targetLowerBound !== TARGET_LOWER_BOUND || payload.targetQuantityPrice !== TARGET_QUANTITY_PRICE) throw new Error("dryRunFingerprint scope mismatch");
  return payload;
}
function isApplied(state) { return state.asin === G83_ASIN && state.normalPrice === EXPECTED_NORMAL_PRICE && state.salePrice === EXPECTED_SALE_PRICE && state.minPrice === EXPECTED_MIN_PRICE && state.b2bPrice === EXPECTED_B2B_PRICE && state.saleEnd === EXPECTED_SALE_END_UTC && state.q10LowerBound === TARGET_LOWER_BOUND && state.q10Price === TARGET_QUANTITY_PRICE && state.quantityLevels.length === 1; }
async function verifyApplied(accessToken) { let lastError = ""; for (let attempt = 1; attempt <= VERIFY_ATTEMPTS; attempt += 1) { try { const state = analyzeListing(await getListing(accessToken)); if (isApplied(state)) return { verified: true, attempt, state }; lastError = `not yet applied: q10=${state.q10Price} levels=${JSON.stringify(state.quantityLevels)}`; } catch (err) { lastError = err?.message || String(err); } if (attempt < VERIFY_ATTEMPTS) await sleep(VERIFY_WAIT_MS); } return { verified: false, attempt: VERIFY_ATTEMPTS, error: lastError }; }

async function handler(req, res) {
  try {
    const secret = getSecret(); if (!secret) return res.status(500).json({ ok: false, error: "AMAZON_STOCK_API_SECRET is not set" });
    if (String(req.headers["x-api-secret"] || "") !== secret) return res.status(401).json({ ok: false, error: "Unauthorized" });
    const dryRun = req.body?.dryRun === true;
    if (!dryRun) { if (req.body?.confirm !== LIVE_CONFIRM) return res.status(400).json({ ok: false, error: `confirm must equal ${LIVE_CONFIRM}` }); verifyFingerprint(req.body?.dryRunFingerprint); }
    if (Number(req.body?.lowerBound) !== TARGET_LOWER_BOUND) return res.status(400).json({ ok: false, error: `lowerBound must equal ${TARGET_LOWER_BOUND}` });
    if (Number(req.body?.quantityPrice) !== TARGET_QUANTITY_PRICE) return res.status(400).json({ ok: false, error: `quantityPrice must equal ${TARGET_QUANTITY_PRICE}` });
    ...
    if (dryRun) {
      ...
      return res.status(200).json({ ok: true, moduleVersion: MODULE_VERSION, status: "DRY_RUN_READY", dryRun: true, sku: G83_SKU, asin: G83_ASIN, before, lowerBound: TARGET_LOWER_BOUND, quantityPrice: TARGET_QUANTITY_PRICE, patchPreview: patchBody, amazonValidation: validation, dryRunFingerprint: makeFingerprint(before), fingerprintTtlMinutes: 60, liveConfirm: LIVE_CONFIRM });
    }
    const accepted = await submitPatch(accessToken, patchBody, false); const verification = await verifyApplied(accessToken);
    if (!verification.verified) return res.status(409).json({ ok: false, moduleVersion: MODULE_VERSION, status: "VERIFICATION_FAILED", accepted, verification });
    return res.status(200).json({ ok: true, moduleVersion: MODULE_VERSION, status: "COMPLETED", dryRun: false, accepted, verification });
  } catch (err) {
    console.error("G83 B2B quantity error", { message: err?.message || String(err), code: err?.code || "" });
...
express.application.post = function g83B2bQuantityPost(path, ...handlers) { if (path === ROUTE) return originalPost.call(this, path, handler); return originalPost.call(this, path, ...handlers); };
