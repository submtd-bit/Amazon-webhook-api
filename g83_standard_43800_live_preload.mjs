...
/**
 * Amazon G83 standard price 43,800 LIVE v1.0.0
 * 2026-08-28
 *
 * Exact approved scope:
 * - SKU E7-YLJ3-F9CY / ASIN B0GZBHBQN2
 * - Consumer normal price only: 58,000 -> 43,800
 * - Preserve B2B 39,900
 * - Preserve quantity discount 10+ units at 3%
 * - Preserve minimum seller allowed price 36,200
 * - Preserve expired discounted-price schedule container
 * - Preserve all non-target purchasable_offer fields
 *
 * Safety flow:
 * Fresh GET -> exact preflight -> VALIDATION_PREVIEW -> Fresh GET again ->
 * exact preflight again -> LIVE PATCH -> Fresh post-verify.
 */
const MODULE_VERSION = "2026-08-28-g83-standard-43800-live-v1.0.0";
const ROUTE = "/amazon/price/g83/standard-43800-live";
const LIVE_CONFIRM = "G83_STANDARD_43800_LIVE_APPROVED_20260828";
...
function beforeLiveBlocks(state) {
  ...
  if (!state.actual.b2cPresent || state.actual.b2cPrice !== TARGET.currentActualB2C) blocks.push(`ACTUAL_B2C_DRIFT:${state.actual.b2cPrice}`);
...
async function handler(req, res) {
  ...
  try {
    ...
    if (String(req.body?.liveConfirm || "") !== LIVE_CONFIRM) return res.status(400).json({ ok: false, moduleVersion: MODULE_VERSION, route: ROUTE, decision: "LIVE_CONFIRM_MISMATCH", externalChanges: 0, error: "LIVE_CONFIRM_MISMATCH" });

    const accessToken = await getLwaAccessToken();

    const initialState = analyzeListing(await getListing(accessToken), Date.now());
    const alreadyApplied = initialState.normalPrice === TARGET.targetNormal && initialState.actual.b2cPrice === TARGET.targetNormal;
    if (alreadyApplied) {
      ...
      if (blocks.length) {
        return res.status(409).json({
          ...
          route: ROUTE,
          decision: "TARGET_PRICE_ALREADY_PRESENT_BUT_PROTECTED_STATE_INVALID",
      ...
      return res.status(200).json({
        ...
        moduleVersion: MODULE_VERSION,
        route: ROUTE,
        decision: "ALREADY_APPLIED_G83_43800",
        validationPreviewCalls: 0,
        liveCalls: 0,
        persistentAmazonWrites: 0,
        externalChanges: 0,
        result: { sku: TARGET.sku, asin: TARGET.asin, targetNormal: TARGET.targetNormal, state: publicState(initialState) },
      });
    }
    ...
    if (initialBlocks.length) {
      return res.status(409).json({
        ...
        moduleVersion: MODULE_VERSION,
        route: ROUTE,
        decision: "LIVE_PREFLIGHT_BLOCKED_NO_MUTATION",
        validationPreviewCalls: 0,
        liveCalls: 0,
        persistentAmazonWrites: 0,
        externalChanges: 0,
    ...
    if (preview.errors.length) {
      return res.status(422).json({
        ...
        moduleVersion: MODULE_VERSION,
        route: ROUTE,
        decision: "LIVE_GATE_VALIDATION_PREVIEW_FAILED_NO_MUTATION",
    ...
    if (secondBlocks.length) {
      return res.status(409).json({
        ...
        moduleVersion: MODULE_VERSION,
        route: ROUTE,
        decision: "POST_PREVIEW_FRESH_DRIFT_NO_LIVE_MUTATION",
        validationPreviewCalls,
        liveCalls: 0,
        persistentAmazonWrites: 0,
        externalChanges: 0,
    ...
    if (live.errors.length) {
      ...
      return res.status(applied ? 200 : 422).json({
        ...
        moduleVersion: MODULE_VERSION,
        route: ROUTE,
        decision: applied ? "LIVE_APPLIED_DESPITE_RESPONSE_ISSUES" : "LIVE_REJECTED_BY_AMAZON",
        validationPreviewCalls,
        liveCalls: 1,
        persistentAmazonWrites: applied ? 1 : 0,
        externalChanges: applied ? 1 : 0,
    ...
    if (!verify.ok) {
      ...
      return res.status(202).json({
        ...
        externalChanges: observedApplied ? 1 : 0,
    ...
    return res.status(200).json({
      ...
      moduleVersion: MODULE_VERSION,
      route: ROUTE,
      decision: "G83_STANDARD_43800_LIVE_PASS",
      approvedChange: {
        sku: TARGET.sku,
        asin: TARGET.asin,
        normalPriceBefore: TARGET.currentNormal,
        normalPriceAfter: TARGET.targetNormal,
      },
      validationPreviewCalls,
      liveCalls: 1,
      ...
      result: {
        state: publicState(finalState),
        pointsBefore: TARGET.amazonPointsBefore,
        pointsAfterObserved: finalState.actual.points,
        pointsChangedObserved: finalState.actual.points !== TARGET.amazonPointsBefore,
      },
      nextStep: "FRESH_V120_READ_AND_PROTECTED_E7_SSOT_RECONCILE",
    });
  } catch (err) {
    console.error("G83 standard 43800 LIVE error", err?.message || String(err));
...
express.application.listen = function g83Standard43800LiveListen(...args) {
