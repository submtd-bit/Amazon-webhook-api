...
/**
 * Amazon G83 standard price 43,800 Validation Preview v1.0.0
 * 2026-08-28
 *
 * Exact scope:
 * - SKU E7-YLJ3-F9CY / ASIN B0GZBHBQN2
 * - Change consumer normal price only: 58,000 -> 43,800
 * - Preserve B2B 39,900 exactly
 * - Preserve quantity discount 10+ at 3% exactly
 * - Preserve minimum seller allowed price 36,200 exactly
 * - Preserve discounted-price schedule container exactly
 * - Preserve every non-target purchasable_offer field exactly
 *
 * VALIDATION_PREVIEW only. No persistent Amazon mutation.
 */
const MODULE_VERSION = "2026-08-28-g83-standard-43800-validation-preview-v1.0.0";
const ROUTE = "/amazon/price/g83/standard-43800-validation-preview";
const BATCH_TOKEN = "G83_STANDARD_43800_20260828_V1";
  ...
  groups.forEach((group, groupIndex) => {
    ...
    schedules.forEach((schedule, scheduleIndex) => {
      ...
      candidates.push({
        ...
        scheduleIndex,
        schedule,
        startMs: epochOrNull(schedule?.start_at) ?? 0,
      });
    });
  });
...
function analyzeListing(listing, nowMs) {
  ...
  const saleDiag = scheduleDiagnostics(consumer, "discounted_price", nowMs);
...
function preflightBlocks(state) {
  ...
  if (state.b2bPrice !== TARGET.b2bPrice) blocks.push(`B2B_ATTRIBUTE_DRIFT:${state.b2bPrice}`);
  if (!state.actual.b2cPresent || state.actual.b2cPrice === null) blocks.push("ACTUAL_B2C_OFFER_MISSING");
  if (state.actual.b2cPrice !== TARGET.currentActualB2C) blocks.push(`ACTUAL_B2C_DRIFT:${state.actual.b2cPrice}`);
  if (!state.actual.b2bPresent || state.actual.b2bPrice === null) blocks.push("ACTUAL_B2B_OFFER_MISSING");
  if (state.actual.b2bPrice !== TARGET.b2bPrice) blocks.push(`ACTUAL_B2B_DRIFT:${state.actual.b2bPrice}`);
  if (!state.actual.pointsPresent || state.actual.points === null) blocks.push("AMAZON_POINTS_MISSING");
  if (state.actual.points !== TARGET.amazonPoints) blocks.push(`POINTS_DRIFT:${state.actual.points}`);
  ...
  const qtyEffective = TARGET.b2bPrice * (1 - TARGET.quantityValue / 100);
  if (TARGET.targetNormal <= TARGET.minimumSellerAllowed) blocks.push("TARGET_NORMAL_NOT_ABOVE_MINIMUM");
  if (TARGET.targetNormal <= TARGET.b2bPrice) blocks.push("TARGET_NORMAL_NOT_ABOVE_B2B");
  if (TARGET.b2bPrice <= TARGET.minimumSellerAllowed) blocks.push("B2B_NOT_ABOVE_MINIMUM");
  if (qtyEffective <= TARGET.minimumSellerAllowed) blocks.push("QTY_EFFECTIVE_NOT_ABOVE_MINIMUM");
  return blocks;
}
...
function buildPreviewOffers(state) {
  ...
  if (!consumer) throw new Error("consumer offer missing while building preview");

  const ref = activeScheduleRef(consumer, "our_price", Date.now());
  if (!ref) throw new Error("active consumer our_price schedule missing while building preview");
  ...
  if (!jsonEqual(afterOffers[state.b2bIndex], beforeB2B)) throw new Error("B2B offer changed while building preview");
  if (!jsonEqual(consumer.discounted_price ?? null, beforeDiscounted)) throw new Error("discounted_price changed while building preview");
  if (!jsonEqual(consumer.minimum_seller_allowed_price ?? null, beforeMinimum)) throw new Error("minimum_seller_allowed_price changed while building preview");
  if (!jsonEqual(consumer.maximum_seller_allowed_price ?? null, beforeMaximum)) throw new Error("maximum_seller_allowed_price changed while building preview");
  if (!jsonEqual(afterOffers[state.b2bIndex]?.quantity_discount_plan ?? null, beforeQuantity)) throw new Error("quantity_discount_plan changed while building preview");
...
async function handler(req, res) {
  ...
  try {
    ...
    if (!secret) return res.status(500).json({ ok: false, moduleVersion: MODULE_VERSION, route: ROUTE, error: "AMAZON_STOCK_API_SECRET is not set", externalChanges: 0 });
    if (String(req.headers["x-api-secret"] || "") !== secret) return res.status(401).json({ ok: false, moduleVersion: MODULE_VERSION, route: ROUTE, error: "Unauthorized", externalChanges: 0 });
    if (String(req.body?.batchToken || "") !== BATCH_TOKEN) return res.status(400).json({ ok: false, moduleVersion: MODULE_VERSION, route: ROUTE, error: "BATCH_TOKEN_MISMATCH", externalChanges: 0 });

    const accessToken = await getLwaAccessToken();
    const listing = await getListing(accessToken);
    const state = analyzeListing(listing, nowMs);
    ...
    if (blocks.length) {
      return res.status(409).json({
        ...
        liveCalls: 0,
        externalChanges: 0,
        failure: { sku: TARGET.sku, asin: TARGET.asin, blocks },
      });
    }

    const built = buildPreviewOffers(state);
    const preview = await validationPreview(accessToken, state.productType, built.offers);
    const validationIssues = summarizeIssues(preview.issues);
    ...
    if (preview.errors.length) {
      return res.status(422).json({
        ...
        moduleVersion: MODULE_VERSION,
        route: ROUTE,
        decision: "VALIDATION_PREVIEW_FAILED",
        approvedCount: 1,
        preflightPassed: 1,
        preflightFailed: 0,
        ...
        result: {
          sku: TARGET.sku,
          asin: TARGET.asin,
          currentNormal: state.normalPrice,
    ...
    return res.status(200).json({
      ...
      moduleVersion: MODULE_VERSION,
      route: ROUTE,
      decision: "VALIDATION_PREVIEW_G83_43800_PASSED",
      approvedCount: 1,
      preflightPassed: 1,
      preflightFailed: 0,
      validationPreviewCalls: 1,
      ...
      result: {
        ...
        asin: TARGET.asin,
        currentNormal: state.normalPrice,
        currentActualB2CPrice: state.actual.b2cPrice,
        targetNormal: TARGET.targetNormal,
        ...
        saleSchedule: {
          ...
          activeCount: state.saleDiagnostics.active,
          expiredCount: state.saleDiagnostics.expired,
          futureCount: state.saleDiagnostics.future,
        },
        mutationMode: "REPLACE_CONSUMER_NORMAL_PRICE_ONLY",
        preservation: built.preservation,
        validationIssues,
      },
    });
  } catch (err) {
    console.error("G83 standard 43800 validation preview error", err?.message || String(err));
    return res.status(500).json({
      ok: false,
      moduleVersion: MODULE_VERSION,
      route: ROUTE,
      decision: "INTERNAL_ERROR_NO_PERSISTENT_MUTATION",
      validationPreviewCalls: 0,
      persistentAmazonWrites: 0,
      liveCalls: 0,
      externalChanges: 0,
...
express.application.listen = function g83Standard43800ValidationPreviewListen(...args) {
