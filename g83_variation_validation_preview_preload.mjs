...
const MODULE_VERSION = "2026-09-04-g83-variation-validation-preview-v1.1.0";
...
function jparse(text){try{return text?JSON.parse(text):{};}catch{return {rawText:String(text||"").slice(0,1600)};}}
...
function rawValues(spec){
  ...
  (function w(n,d){
    ...
    if(Array.isArray(n)){n.forEach(x=>w(x,d+1));return;}
...
function relation(a){return {parentageLevel:first(a?.parentage_level),parentSku:a?.child_parent_sku_relationship?.[0]?.parent_sku??null,childRelationshipType:a?.child_parent_sku_relationship?.[0]?.child_relationship_type??null,variationTheme:a?.variation_theme?.[0]?.name??null};}
function offerSummary(x){return (Array.isArray(x?.offers)?x.offers:[]).map(r=>({offerType:r?.offerType||"",amount:r?.price?.amount??null,currency:r?.price?.currencyCode||r?.price?.currency||"",points:r?.points?.pointsNumber??null,quantityDiscountPlan:r?.quantityDiscountPlan||null}));}
function imageCount(a){return Object.keys(a||{}).filter(k=>/(main_product_image_locator|other_product_image_locator|swatch_product_image_locator)/i.test(k)).reduce((n,k)=>n+(Array.isArray(a[k])?a[k].length:0),0);}
function flatten(v,out=[]){if(v==null)return out;if(typeof v==="string"){out.push(v);return out;}if(Array.isArray(v)){v.forEach(x=>flatten(x,out));return out;}if(typeof v==="object")Object.values(v).forEach(x=>flatten(x,out));return out;}
function markerKeys(attrs,re){
  const out=[];
  for(const [k,v] of Object.entries(attrs||{})){const text=flatten(v,[]).join(" ");if(re.test(text))out.push(k);}
  return out.sort();
}
function bundle(a,title){
  ...
  return {office2024:/OFFICE\s*2024|MICROSOFT\s*OFFICE\s*2024|MS\s*OFFICE\s*2024/.test(b),norton360:/NORTON\s*360|ノートン\s*360/.test(b),wps:/WPS\s*OFFICE|WPSOFFICE/.test(b)};
}
function inspect(plan,r){
  ...
  return {plan,attrs:a,preserve:{sku:plan.sku,asin:plan.asin,title,status:Array.isArray(s?.status)?s.status:[],availableQuantity:qty(x),ramGB:rg,storageGB:sg,
    ...
    issueCodes:[...new Set(issues.map(i=>String(i?.code||"")).filter(Boolean))],bundle:bundle(a,title),wpsAttributeKeys:markerKeys(a,/WPS\s*OFFICE|WPSOFFICE/i)}};
}
function relationRows(kind,relationship){
  ...
  if(kind==="child")o.child_parent_sku_relationship=[{marketplace_id:MARKETPLACE_ID,child_relationship_type:relationship,parent_sku:PARENT_SKU}];
  return o;
}
function attrPatch(attrs,key,value){
  return {op:Array.isArray(attrs?.[key])&&attrs[key].length?"replace":"add",path:`/attributes/${key}`,value};
}
...
function exclusiveFalse(schema){
  const spec=nestedSpec(schema,"is_exclusive_product","value");if(!spec)throw new Error("PTD missing is_exclusive_product.value");
  const allowed=rawValues(spec);let value=null;
  ...
  if(value===null)throw new Error(`PTD no false value for is_exclusive_product allowed=${JSON.stringify(allowed.slice(0,10))}`);
...
function cloneTemplate(sourceAttrs,canonicalAttrs,key){
  ...
  if(!row||typeof row!=="object"||Array.isArray(row)||!Object.prototype.hasOwnProperty.call(row,"value"))throw new Error(`CONTENT_TEMPLATE_MISSING ${key}`);
  return clone(row);
}
...
function titleFor(plan){return `【整備済み品】ダイナブック G83/HS 中古ノートパソコン 13.3型FHD 第11世代 Core i5-1135G7 メモリ${plan.memoryGB}GB SSD${storageLabel(plan.storageGB)} Windows 11 Pro MS Office 2024 Webカメラ Wi-Fi6 ノートン360付属 MTD整備済み`;}
...
function parentAttrs(source,relationship,exclusive){
  ...
  for(const k of ["externally_assigned_product_identifier","merchant_suggested_asin","purchasable_offer","fulfillment_availability","condition_type","list_price",
  ...
  const r=relationRows("parent",relationship);a.parentage_level=r.parentage_level;a.variation_theme=r.variation_theme;a.is_exclusive_product=clone(exclusive);
  return a;
}
...
function sum(r){
  const issues=Array.isArray(r?.body?.issues)?r.body.issues:[],errors=issues.filter(i=>String(i?.severity||"").toUpperCase()==="ERROR"),status=String(r?.body?.status||"").toUpperCase();
  return {httpStatus:r.http,responseOk:r.ok,status,submissionId:r?.body?.submissionId||"",issueCount:issues.length,errorCount:errors.length,
    issueCodes:[...new Set(issues.map(i=>String(i?.code||"")).filter(Boolean))],
    errors:errors.slice(0,12).map(i=>({code:String(i?.code||""),message:String(i?.message||"").slice(0,600),attributeNames:Array.isArray(i?.attributeNames)?i.attributeNames:[]})),
    valid:r.ok&&errors.length===0&&["VALID","ACCEPTED"].includes(status)};
}
async function handler(req0,res){
  try{
    const sec=secret();
    if(!sec)return res.status(500).json({ok:false,moduleVersion:MODULE_VERSION,route:ROUTE,externalChanges:0,error:"secret missing"});
    ...
    const relationship=vals(nestedSpec(schema,"child_parent_sku_relationship","child_relationship_type")).find(v=>/^variation$/i.test(v));
    ...
    if(canonical.preserve.errorCount>0)throw new Error(`CANONICAL_E7_HAS_ERRORS ${canonical.preserve.issueCodes.join(",")}`);
    if(!canonical.preserve.bundle.office2024||!canonical.preserve.bundle.norton360||canonical.preserve.bundle.wps)throw new Error(`CANONICAL_E7_BUNDLE_UNEXPECTED ${JSON.stringify(canonical.preserve.bundle)}`);
    ...
    for(const child of kids.filter(x=>!x.plan.canonical)){
      ...
      if(outside.length)unresolved.push({sku:child.plan.sku,asin:child.plan.asin,keys:outside,reason:"WPS_MARKER_OUTSIDE_ALLOWED_CONTENT_KEYS"});
    }

    const parent=sum(await putPreview(a,PARENT_SKU,parentAttrs(canonical.attrs,relationship,exclusive)));
    const childPreviews=[];
    for(const child of kids){
      ...
      childPreviews.push({
        ...
        relationOnly:child.plan.canonical,
        patchPaths:patches.map(p=>p.path),
        targetBundle:{office2024:true,norton360:true,wps:false},
        preview:sum(await patchPreview(a,child.plan.sku,patches))
      });
    }

    const technicalReady=parent.valid&&childPreviews.every(x=>x.preview.valid);
    const contentDesignReady=unresolved.length===0;
    const blockers=[];
    if(!parent.valid)blockers.push("PARENT_VALIDATION_PREVIEW_FAILED");
    for(const x of childPreviews)if(!x.preview.valid)blockers.push(`CHILD_VALIDATION_PREVIEW_FAILED:${x.sku}`);
    for(const x of unresolved)blockers.push(`UNRESOLVED_WPS_ATTRIBUTE:${x.sku}:${x.keys.join(",")}`);

    const writes=[
      {type:"PUT_NEW_PARENT",sku:PARENT_SKU,scope:["parentage_level","variation_theme","is_exclusive_product","parent content cloned from E7 with RAM/SSD/offer/inventory removed"]},
      ...kids.map(x=>({
        type:"PATCH_CHILD",
        sku:x.plan.sku,
        asin:x.plan.asin,
        scope:x.plan.canonical
          ?["parentage_level","child_parent_sku_relationship","variation_theme","is_exclusive_product"]
          :["parentage_level","child_parent_sku_relationship","variation_theme","is_exclusive_product","item_name","bullet_point","product_description","included_components",...(x.preserve.wpsAttributeKeys.includes("software_included")?["software_included"]:[])]
      }))
    ];

    return res.status(200).json({
      ok:true,moduleVersion:MODULE_VERSION,route:ROUTE,status:technicalReady&&contentDesignReady?"PASS":"BLOCK",productType:PRODUCT_TYPE,parentSku:PARENT_SKU,parentTitle:PARENT_TITLE,
      variationTheme:VARIATION_THEME,isExclusiveProductTarget:false,
      preserveSnapshot:kids.map(x=>x.preserve),
      contentHarmonization:{
        canonicalSku:canonical.plan.sku,
        canonicalBundle:canonical.preserve.bundle,
        targetBundle:{office2024:true,norton360:true,wps:false},
        oldChildrenTargeted:kids.filter(x=>!x.plan.canonical).map(x=>({sku:x.plan.sku,asin:x.plan.asin,title: titleFor(x.plan),wpsAttributeKeys:x.preserve.wpsAttributeKeys})),
        unresolved
      },
      validationPreview:{parent,children:childPreviews},
      prospectivePersistentWritesIfSeparatelyApproved:{
        count:7,writes,
        deferredNotIncluded:["old TJ-00SX-UW3J parent retirement","child price changes","child inventory changes","child B2B changes","child image changes","Amazon Ads","Yahoo"]
      },
      decision:{
        technicalReady,contentDesignReady,readyForLiveDesign:technicalReady&&contentDesignReady,blockers,
        next:technicalReady&&contentDesignReady?"Prepare exact guarded LIVE route and request explicit approval.":"Do not LIVE. Resolve reported blockers first."
      },
      readOnlyAmazonPersistence:true,externalChanges:0,amazonPersistentWrites:0,priceWrites:0,inventoryWrites:0,adsWrites:0,yahooWrites:0,
      liveAllowed:false,liveBlockedReason:"EXPLICIT_USER_LIVE_APPROVAL_REQUIRED"
    });
  }catch(err){
    return res.status(400).json({ok:false,moduleVersion:MODULE_VERSION,route:ROUTE,externalChanges:0,amazonPersistentWrites:0,priceWrites:0,inventoryWrites:0,adsWrites:0,yahooWrites:0,error:err?.message||String(err)});
  }
}
express.application.listen=function g83VariationValidationPreviewListen(...args){
