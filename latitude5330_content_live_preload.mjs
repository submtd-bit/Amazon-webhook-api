...
const MODULE_VERSION = "2026-08-26-latitude5330-content-live-v1.0.0";
const ROUTE = "/amazon/listing/latitude5330-content-live";
...
const GUARD = Object.freeze({
  ...
  approvedPreviewSubmissionId: "c395cea58e1d4b34ae3e3b01d31bc6c7",
  confirmLive: "CONFIRM_LATITUDE5330_CONTENT_B0HGDZNVQN_20260826",
...
function numEq(actual, expected) { return Number.isFinite(Number(actual)) && Number(actual) === Number(expected); }
function assertEqual(label, actual, expected) {
  if (String(actual ?? "") !== String(expected ?? "")) throw new Error(`CONTENT_SOURCE_DRIFT: ${label} expected=${JSON.stringify(expected)} actual=${JSON.stringify(actual)}`);
}
...
function assertSource(listing) {
  ...
  if (!numEq(readOfferPrice(listing, "B2C"), GUARD.b2cPrice)) throw new Error(`CONTENT_SOURCE_DRIFT: B2C price expected=${GUARD.b2cPrice} actual=${readOfferPrice(listing, "B2C")}`);
  if (!numEq(readOfferPrice(listing, "B2B"), GUARD.b2bPrice)) throw new Error(`CONTENT_SOURCE_DRIFT: B2B price expected=${GUARD.b2bPrice} actual=${readOfferPrice(listing, "B2B")}`);
  if (!numEq(readDefaultQuantity(listing), GUARD.quantity)) throw new Error(`CONTENT_SOURCE_DRIFT: quantity expected=${GUARD.quantity} actual=${readDefaultQuantity(listing)}`);
...
function verifyTarget(listing) {
  ...
  const checks = {
    ...
    b2cPriceStill59800: numEq(readOfferPrice(listing, "B2C"), GUARD.b2cPrice),
    b2bPriceStill56000: numEq(readOfferPrice(listing, "B2B"), GUARD.b2bPrice),
    quantityStill0: numEq(readDefaultQuantity(listing), GUARD.quantity),
  };
  return { verified: Object.values(checks).every(Boolean), checks, snapshot: { bulletPoints: bullets, description: firstValue(attributes, "product_description"), specificUse: firstValue(attributes, "specific_uses_for_product"), conditionType: firstValue(attributes, "condition_type"), brand: firstValue(attributes, "brand"), manufacturer: firstValue(attributes, "manufacturer"), warranty: firstValue(attributes, "warranty_description"), offers: listing?.offers || [], fulfillmentAvailability: listing?.fulfillmentAvailability || [] } };
}
...
async function handler(req, res) {
  ...
  try {
    ...
    livePatchSent = true;
    if (!liveResult.valid) return res.status(502).json({ ok: false, moduleVersion: MODULE_VERSION, route: ROUTE, sku, asin: GUARD.asin, livePatchSent: true, livePatchAttempts: 1, freshPreview, liveResult, postVerified: false, externalChanges: 0, error: "LIVE_PATCH_RESPONSE_NOT_ACCEPTED" });
    ...
    return res.status(200).json({ ok: postVerified, moduleVersion: MODULE_VERSION, route: ROUTE, sku, asin: GUARD.asin, productType: GUARD.productType, approvedPreviewSubmissionId: GUARD.approvedPreviewSubmissionId, freshPreview, livePatchSent: true, livePatchAttempts: 1, liveResult, postVerified, verificationAttempts, finalSnapshot: verification?.snapshot || {}, externalChanges: postVerified ? 1 : 0, verificationPending: !postVerified, note: postVerified ? "Latitude 5330 sales content updated and verified; condition, prices and quantity preserved" : "LIVE PATCH was accepted once, but propagation was not fully visible within the verification window; do not retry LIVE automatically" });
  } catch (err) {
    console.error("Latitude 5330 content live error", err?.message || String(err));
...
express.application.listen = function latitude5330ContentLiveListen(...args) {
