...
const MODULE_VERSION = "2026-08-29-latitude5330-variation-validation-preview-v1.0.0";
...
function chooseTheme(arr){const ranked=arr.filter(v=>/(hard.*disk|disk.*size|storage|capacity|hard_disk)/i.test(v));if(ranked.length===1)return ranked[0];throw new Error(`SCHEMA_AMBIGUOUS variation_theme=${JSON.stringify(arr)}`);}
...
function buildParent(src,rel){const a=clone(src);stripIdentity(a);stripOfferForParent(a);delete a.hard_disk;delete a.flash_memory;setValue(a,"item_name","【整備済み品】デル Latitude 5330 13.3型 i5-1245U 16GB Win11 Pro ノートン・Office付");commonImage(a);a.parentage_level=rel.parentage_level;a.variation_theme=rel.variation_theme;return a;}
function build512(src,rel){const a=clone(src);stripIdentity(a);a.hard_disk=replaceSizeRows(a.hard_disk,"hard_disk",G.targetGB);a.flash_memory=replaceSizeRows(a.flash_memory,"flash_memory",G.targetGB);setValue(a,"item_name","【整備済み品】デル Latitude 5330 13.3型 i5-1245U 16GB SSD512GB Win11 Pro ノートン・Office付");commonImage(a);Object.assign(a,rel);return a;}
function child256Patches(rel){return Object.entries(rel).map(([k,v])=>({op:"add",path:`/attributes/${k}`,value:v}));}
...
function sum(r){const issues=Array.isArray(r?.body?.issues)?r.body.issues:[];const errors=issues.filter(i=>String(i?.severity||"").toUpperCase()==="ERROR");return {httpStatus:r.http,responseOk:r.ok,status:String(r?.body?.status||""),submissionId:String(r?.body?.submissionId||""),issueCount:issues.length,errorCount:errors.length,issueCodes:[...new Set(issues.map(i=>String(i?.code||"")).filter(Boolean))],errors:errors.slice(0,12).map(i=>({code:String(i?.code||""),message:String(i?.message||"").slice(0,450),attributeNames:Array.isArray(i?.attributeNames)?i.attributeNames:[]})),valid:r.ok&&errors.length===0&&["VALID","ACCEPTED"].includes(String(r?.body?.status||"").toUpperCase())};}
async function handler(req0,res){try{
  const sec=secret();if(!sec)return res.status(500).json({ok:false,readOnly:true,externalChanges:0,error:"secret missing"});if(String(req0.headers["x-api-secret"]||"")!==sec)return res.status(401).json({ok:false,readOnly:true,externalChanges:0,error:"Unauthorized"});if(req0.body?.dryRun===false)throw new Error("LIVE disabled");
  ...
  const a=await token();const src=assertSource(await listing(a));const s=await schema(a);
  ...
  const parentValue=pickEnum(parentVals,/^parent$/i,"parentage parent");const childValue=pickEnum(parentVals,/^child$/i,"parentage child");const relationshipValue=pickEnum(relVals,/variation/i,"child_relationship_type");const theme=chooseTheme(themeVals);
  const parentage={parent:parentValue,child:childValue};const parentRel=relationRows(parentValue,parentage,relationshipValue,theme);const childRel=relationRows(childValue,parentage,relationshipValue,theme);
  const child256=sum(await patchPreview(a,G.sourceSku,child256Patches(childRel)));
  const parent=sum(await putPreview(a,G.parentSku,buildParent(src.attributes,parentRel)));
  const child512=sum(await putPreview(a,G.child512Sku,build512(src.attributes,childRel)));
  const ready=child256.valid&&parent.valid&&child512.valid;
  return res.status(200).json({ok:true,moduleVersion:MODULE_VERSION,route:ROUTE,readOnly:true,externalChanges:0,guards:{sourceSku:G.sourceSku,sourceAsin:G.sourceAsin,parentSku:G.parentSku,child512Sku:G.child512Sku,productType:G.productType,commonImageUrl:G.commonImageUrl,price512Reference:G.price512},schemaSelection:{parentageEnum:parentVals,childRelationshipTypeEnum:relVals,variationThemeEnum:themeVals,selected:{parent:parentValue,child:childValue,relationship:relationshipValue,theme}},preview:{parent,child256,child512},readyForLiveDesign:ready,next:ready?"Prepare guarded LIVE sequence: parent -> child256 relation -> child512, then Fresh verify before unpausing inventory.":"STOP. Inspect validation errors; no live mutation."});
