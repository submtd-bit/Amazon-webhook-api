...
const MODULE_VERSION = "2026-08-29-latitude5330-variation-validation-preview-v1.1.0";
...
function child256Patches(rel){const patches=Object.entries(rel).map(([k,v])=>({op:"add",path:`/attributes/${k}`,value:v}));patches.push({op:"add",path:"/attributes/main_product_image_locator",value:[{media_location:G.commonImageUrl,marketplace_id:MARKETPLACE_ID}]});return patches;}
...
function sum(r){const issues=Array.isArray(r?.body?.issues)?r.body.issues:[];const errors=issues.filter(i=>String(i?.severity||"").toUpperCase()==="ERROR");return {httpStatus:r.http,responseOk:r.ok,status:String(r?.body?.status||""),issueCount:issues.length,errorCount:errors.length,issueCodes:[...new Set(issues.map(i=>String(i?.code||"")).filter(Boolean))],errors:errors.slice(0,6).map(i=>({code:String(i?.code||""),message:String(i?.message||"").slice(0,300),attributeNames:Array.isArray(i?.attributeNames)?i.attributeNames:[]})),valid:r.ok&&errors.length===0&&["VALID","ACCEPTED"].includes(String(r?.body?.status||"").toUpperCase())};}
function storageThemes(all){
  const exact=new Set(all.filter(v=>String(v).split("/").includes("HARD_DISK_SIZE")));
  const preferred=["HARD_DISK_SIZE/RAM_MEMORY_INSTALLED_SIZE","DISPLAY_SIZE/HARD_DISK_SIZE","GRAPHICS_COPROCESSOR/HARD_DISK_SIZE","COMPUTER_MEMORY_SIZE/HARD_DISK_SIZE","DISPLAY_SIZE/RAM_MEMORY_INSTALLED_SIZE/HARD_DISK_SIZE","HARD_DISK_SIZE/PROCESSOR_DESCRIPTION/RAM_MEMORY_INSTALLED_SIZE"];
...
async function handler(req0,res){try{
  ...
  for(const theme of candidates){const childRel=relationRows(childValue,parentage,relationshipValue,theme);const p=sum(await patchPreview(a,G.sourceSku,child256Patches(childRel)));probes.push({theme,valid:p.valid,httpStatus:p.httpStatus,errorCount:p.errorCount,issueCodes:p.issueCodes,errors:p.errors});if(p.valid){selected=theme;break;}}
  if(!selected)return res.status(200).json({ok:true,moduleVersion:MODULE_VERSION,route:ROUTE,readOnly:true,externalChanges:0,status:"BLOCK",reason:"NO_VALID_STORAGE_THEME_IN_PROBE_SET",schemaSelection:{parent:parentValue,child:childValue,relationship:relationshipValue,candidateCount:candidates.length,candidates},themeProbes:probes,readyForLiveDesign:false,next:"STOP. Review compact theme probe errors; no live mutation."});
