...
const MODULE_VERSION = "2026-08-25-s73-listing-live-v1.0.0";
const ROUTE = "/amazon/listing/s73-live";
...
const GUARD = Object.freeze({
  ...
  approvedPreviewSubmissionId: "964454a386004d259673828b01e1e348",
  confirmLive: "CONFIRM_S73_B0HGDBYRS8_20260825",
  source: Object.freeze({
    title: "【整備済み品】ダイナブック S73/HS 13.3型 Core i5-11世代 16GB SSD256GB Win11 Pro ノートン、Office付",
    cpuFamily: "intel_core_i5_1135g7",
    cpuManufacturer: "Intel",
    cpuModelNumber: "10310U",
    cpuSpeedGHz: 2.21,
    cpuGeneration: "第11世代",
    displayInches: 12.1,
    displayType: "LED",
...
function assertIdentityAndSource(listing) {
  ...
  assertEqual("cpu.generation", nestedValue(attributes, "cpu_model", "generation"), s.cpuGeneration);
  ...
  // Fields intentionally not changed by this operation. Guard the important facts we already verified.
  ...
  assertNumber("lithium_battery.energy_content", nestedValue(attributes, "lithium_battery", "energy_content"), 42);
...
function buildPatches(attributes) {
  ...
  patches.push({
    ...
    value: replaceValueAttribute(attributes, "item_name", t.title),
  });

  const cpu = requireArrayAttribute(attributes, "cpu_model");
  assertEqual("cpu.family.before_build", cpu?.[0]?.family?.[0]?.value, t.cpuFamily);
  assertEqual("cpu.generation.before_build", cpu?.[0]?.generation?.[0]?.value, t.cpuGeneration);
...
function verifyTarget(listing) {
  ...
  const checks = {
    ...
    title: String(directValue(attributes, "item_name") || "") === t.title,
    ...
    cpuGeneration: String(nestedValue(attributes, "cpu_model", "generation") || "") === t.cpuGeneration,
    ...
    batteryStill42Wh: numEq(nestedValue(attributes, "lithium_battery", "energy_content"), 42),
...
async function handler(req, res) {
  ...
  try {
    ...
    // Fresh GET immediately before any mutation. Exact source-state drift checks fail closed.
    ...
    // Fresh validation on the exact payload that would be sent live.
    ...
    // Exactly one LIVE PATCH. Never retry this mutation automatically.
    ...
    // Read-only post verification. Do not resend PATCH if propagation is delayed.
    ...
    return res.status(200).json({
      ...
      note: postVerified ? "S73 listing attributes updated and verified" : "LIVE PATCH was accepted once, but propagation was not fully visible within the verification window; do not retry LIVE automatically",
    });
  } catch (err) {
    console.error("S73 listing live error", err?.message || String(err));
...
express.application.listen = function s73ListingLiveListen(...args) {
