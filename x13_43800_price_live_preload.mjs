...
const MODULE_VERSION = "2026-09-03-x13-43800-price-v1.0.0";
const DRY_ROUTE = "/amazon/price/x13-43800/dry-run";
const LIVE_ROUTE = "/amazon/price/x13-43800/live";
const LIVE_CONFIRM = "X13_43800_LIVE_APPROVED_20260903";
...
const TARGET = Object.freeze({
  ...
  maximumSellerAllowed: 63000,
  amazonPointsBefore: 630,
  b2bAttributePrice: 59850,
  ...
  try {
    ...
    if (initialState.normalPrice === TARGET.targetNormal && initialState.actual.b2cPrice === TARGET.targetNormal) {
      ...
      return res.status(blocks.length ? 409 : 200).json({
        ...
        moduleVersion: MODULE_VERSION,
        route: DRY_ROUTE,
        decision: blocks.length ? "TARGET_ALREADY_PRESENT_BUT_PROTECTED_STATE_INVALID" : "ALREADY_APPLIED_X13_43800",
    ...
    return res.status(200).json({
      ...
      moduleVersion: MODULE_VERSION,
      route: DRY_ROUTE,
      decision: "DRY_RUN_READY_FOR_X13_43800_NO_MUTATION",
      approvedScope: {
        sku: TARGET.sku,
        asin: TARGET.asin,
        currentNormal: TARGET.currentNormal,
    ...
    console.error("X13 43800 dry-run error", err?.message || String(err));
...
async function liveHandler(req, res) {
  ...
  try {
    ...
    console.error("X13 43800 live error", err?.message || String(err));
    return res.status(500).json({
      ok: false,
      moduleVersion: MODULE_VERSION,
      route: LIVE_ROUTE,
...
express.application.listen = function x1343800PriceListen(...args) {
